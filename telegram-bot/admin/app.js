      ...
      .map((lead) => {
        const assigneeOptions = ['<option value="">Без менеджера</option>']
          .concat(
            usersForAssign().map(
              (u) =>
                `<option value="${u.id}" ${lead.assignee_user_id === u.id ? "selected" : ""}>${escapeHtml(u.username)}</option>`
            )
          )
          .join("");
...
function renderActivity(logs) {
  els.activityList.innerHTML = logs
    .map(
      (log) =>
        `<div class="log-item"><b>${escapeHtml(log.created_at)}</b> | ${escapeHtml(log.actor_username || "system")} | ${escapeHtml(log.action)}<br><small>${escapeHtml(log.details || "")}</small></div>`
    )
    .join("");
}
